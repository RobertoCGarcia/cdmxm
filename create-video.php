...
if ($http_code == 200) {
        //User login into the website and with autenticate session
        print " ==200 code \n";
        ...
        // REST Server URL
        $request_url = 'https://dev.eluniversal.com.mx/euvideo_api/node.json';
        ...
        $node_data = array(
          ...
          'title' => 'titulo de prueba video',
          'field_titulo_abreviado' => 'titulo de prueba video',
          'field_titulo_seo' => 'Titulo SEO de la nota especifica',//':[{"value":"cewcxew  cwe xew ew xwe ew"}]',
          'field_seccion' => 1,
          'field_thumbnail' => "https://i.ytimg.com/vi/KSbq8zFaIhU/mqdefault.jpg?sqp=CITL7vEF&rs=AOn4CLCGgoQgauGSEcEgLwW06SeczHxipQ",
          'field_resumen' => 'Resumen',
          'field_video' => 'Video',
          'field_id_del_video[und][0][value]' => '3423',//'[{"value": "123","safe_value": "123","format" : "string"}],',
          'field_tags' => 'prueba,test',
          'field_titulo_seo[und][0][value]' => 'field_titulo_seo',
        );

        //$node_data = http_build_query($node_data);
        $strXCSRFTOken = 'X-CSRF-Token: ' . $logged_user["token"];
        ...
        $curl = curl_init($request_url);
        curl_setopt($curl, CURLOPT_HTTPHEADER, array('Content-Type: application/json')); // Accept JSON response
        curl_setopt($curl, CURLOPT_HTTPHEADER, array($strXCSRFTOken) ); // Accept JSON response
        curl_setopt($curl, CURLOPT_POST, 1); // Do a regular HTTP POST
        ...
        if ($http_code == 200) {
             print "insert node ";
             // Convert json response as array
             $node = json_decode($responseInsert, true);
             print_r($node);
        }else{
             // Get error msg
             print "no insert node ";
